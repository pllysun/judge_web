...
export const codeRunService = {
  ...
  async runCode(code: any, language: string, input: string): Promise<CodeRunResult> {
    try {
      // 处理响应式对象，提取实际代码值
      const codeValue = typeof code === 'object' && code !== null && '_value' in code ? code._value : code;
   ...
   * @returns 所有测试用例的运行结果
   */
  async runMultipleTestCases(code: string, language: string, testCases: Array<{input: string, expectedOutput: string}>): Promise<Array<{result: CodeRunResult, expectedOutput: string, passed: boolean}>> {
    const results = [];
